...
class BERTModel(TransformersModel):
    ...
    def token_is_sep(self, token):
        return (token == self.tokenizer.cls_token_id or 
    ...
    def get_targeted_output(self, texts):
        ...
        assert mask_indices.shape[0] == inputs.shape[0], f"Likely some input does not have a mask token. Input: {texts}"
    ...
    def get_output(self, texts, return_attn_mask=False, 
        ...
        #print('right before warning', attn_mask.dtype)
        last_non_masked_idx = torch.sum(attn_mask, dim=1) - 1
